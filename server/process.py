...
def find_anchors(model, train_loader, sample_id, no_val):
    ...
    while (iterations > 0):
        ...
        if (max_ind[0] >= treshold):
            ...
            print(f'id: {sample_id}\t anchor: {feature}')
            return mask
        iterations -= 1
...
def verify_anchors_tabular(model,exp,train_set,X_train,idx):
    # print('Anchor: %s' % (' AND '.join(exp.names())))
    print('Anchor: ' , exp.features())
...
def anchors_tabular(model,idx,train_set,test_set):
    ...
    verify_anchors_tabular(model,exp,train_set,X_train,idx)
    return exp.features()
